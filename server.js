...
// Current holdings derived from full transaction history (Investment_Records_for_Claude.xlsx)
// Net shares = total bought - total sold across all dated transactions
const HOLDINGS = [
  // STOCKS
  ...
  // CRYPTO
  { ticker:'BTC',   name:'Bitcoin',                type:'crypto',   shares:0.0672, avgCost:101992.22 },
...
// Full transaction history for history pages
const TRANSACTIONS = [
  // STOCKS - winners
  ...
  { symbol:'NVDA',  name:'Nvidia (partial)',         category:'stock',     spent:748,   received:207,   pl:-541,   status:'partial'},
  ...
  { symbol:'WAL',   name:'Western Alliance',        category:'stock',     spent:1274,  received:1403,  pl:129,    status:'closed' },
  { symbol:'Z',     name:'Zillow',                  category:'stock',     spent:1589,  received:1685,  pl:96,     status:'closed' },
  { symbol:'SNOW',  name:'Snowflake',               category:'stock',     spent:1646,  received:1692,  pl:46,     status:'closed' },
  { symbol:'ENTG',  name:'Entegris (partial)',       category:'stock',     spent:669,   received:680,   pl:11,     status:'partial'},
  // STOCKS - losers
  ...
  { symbol:'PPTA',  name:'Perpetua Resources',      category:'stock',     spent:1659,  received:1392,  pl:-267,   status:'closed' },
  { symbol:'CLS',   name:'Celestica',               category:'stock',     spent:1555,  received:1400,  pl:-155,   status:'closed' },
  { symbol:'CYBR',  name:'CyberArk',                category:'stock',     spent:1608,  received:1500,  pl:-108,   status:'closed' },
  ...
  { symbol:'MSTR',  name:'MicroStrategy (partial)', category:'stock',     spent:3709,  received:3314,  pl:-395,   status:'partial'},
  { symbol:'BABA',  name:'Alibaba',                 category:'stock',     spent:5555,  received:3946,  pl:-1609,  status:'closed' },
  // STOCKS - open (no realized PL yet)
  { symbol:'EOSE',  name:'Eos Energy',              category:'stock',     spent:3171,  received:2905,  pl:0,      status:'open'   },
...
// Finnhub ticker mapping (some tickers differ)
const TICKER_MAP = { 'QNTM':'QNTM', 'SEC0':'SEC0', 'IEMG':'IEMG', 'BTC':'BINANCE:BTCUSDT', 'ETH':'BINANCE:ETHUSDT', 'XRP':'BINANCE:XRPUSDT', 'NEXO':'NEXO', 'RND':'RENDER-USD' };

function delay(ms) { return new Promise(function(r){setTimeout(r,ms);}); }

async function fetchQuote(ticker) {
  var fticker = TICKER_MAP[ticker] || ticker;
  var url = 'https://finnhub.io/api/v1/quote?symbol=' + fticker + '&token=' + FINNHUB_KEY;
  try {
    ...
    if (data && data.c && data.c > 0) {
      ...
      priceCache.stocks[ticker] = {price:data.c, change:change, changePct:changePct, prevClose:data.pc, high:data.h, low:data.l, open:data.o};
      return true;
    }
...
async function fetchAllPrices() {
  ...
  console.log('Done. '+Object.keys(priceCache.stocks).length+'/'+tickers.length+' loaded.');
}
...
function buildPortfolio() {
  var enriched = HOLDINGS.map(function(h) {
    ...
    return Object.assign({},h,{price:price,value:value,cost:cost,pl:pl,plPct:plPct,changePct:q.changePct||0,change:q.change||0,high:q.high,low:q.low,liveData:!!q.price});
  });
...
function buildSummary(portfolio) {
  ...
  return {totalValue:totalValue,totalCost:totalCost,totalPL:totalPL,plPct:(totalPL/totalCost)*100,dayChange:dayChange,dayChangePct:(dayChange/totalValue)*100,cash:CASH,cashPct:(CASH/totalValue)*100,positions:portfolio.length,winners:portfolio.filter(function(h){return h.pl>0;}).length,losers:portfolio.filter(function(h){return h.pl<0;}).length,liveCount:portfolio.filter(function(h){return h.liveData;}).length,updatedAt:priceCache.updatedAt};
}
...
app.get('/api/allocation', function(req,res){var p=buildPortfolio();var s=buildSummary(p);var alloc=p.map(function(h){return{ticker:h.ticker,name:h.name,type:h.type,value:h.value,pct:h.allocPct};}).concat([{ticker:'CASH',name:'Cash',type:'cash',value:CASH,pct:s.cashPct}]);res.json({ok:true,totalValue:s.totalValue,allocation:alloc});});
app.get('/api/transactions', function(req,res){var cat=req.query.category;var data=cat?TRANSACTIONS.filter(function(t){return t.category===cat;}):TRANSACTIONS;var ts=data.reduce(function(a,t){return a+t.spent;},0),tr=data.reduce(function(a,t){return a+t.received;},0),tp=data.filter(function(t){return t.received>0;}).reduce(function(a,t){return a+t.pl;},0);res.json({ok:true,transactions:data,summary:{totalSpent:ts,totalReceived:tr,totalPL:tp,count:data.length}});});
app.get('/api/transactions/categories', function(req,res){var cats=['stock','etf','crypto','commodity'];var result=cats.map(function(cat){var data=TRANSACTIONS.filter(function(t){return t.category===cat;});var spent=data.reduce(function(a,t){return a+t.spent;},0),received=data.reduce(function(a,t){return a+t.received;},0),pl=data.filter(function(t){return t.received>0;}).reduce(function(a,t){return a+t.pl;},0);return{category:cat,count:data.length,totalSpent:spent,totalReceived:received,realizedPL:pl,plPct:spent>0?(pl/spent)*100:0,winners:data.filter(function(t){return t.pl>0;}).length,losers:data.filter(function(t){return t.pl<0;}).length};});res.json({ok:true,categories:result});});
...
app.get('/api/health', function(req,res){var loaded=Object.keys(priceCache.stocks).length;res.json({ok:true,status:'running',tickersLoaded:loaded,totalTickers:HOLDINGS.length,updatedAt:priceCache.updatedAt});});
app.get('*', function(req,res){res.sendFile(path.join(__dirname,'public','index.html'));});
app.listen(PORT, function(){console.log('Portfolio platform running on port '+PORT);});
